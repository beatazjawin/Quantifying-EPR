...
function output=LOSR_conversions_robust_channel_X2(din,dout,J1,J2)
...
for Y=1:2
 ...
 % conditions on the maps J and F
        ...
        for lambda=1:64 
           J(:,:,lambda) + k*eye(16) == hermitian_semidefinite(din*din*dout*dout);
        end
        
        for lambda=1:64
            F(:,:,lambda) + k*eye(4) == hermitian_semidefinite(din*din);
        end
